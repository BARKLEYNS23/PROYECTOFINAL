...
class Database
{
    ...
    {
        $conexion = Database::getInstance();
        $sql = "SELECT id,nombre,horas from materias"; 
        $result = $conexion->db->prepare($sql);
        $result->execute();
        return $result;
    }
    ...
    public function CrearMateria($nombre,$horas)  
    {
        try {
            $conexion = Database::getInstance();
            $result = $conexion->db->prepare("INSERT INTO materias (nombre,horas) VALUES (:nombre,:horas)"); 
            $result->execute(
                array(
                    ':nombre' => $nombre,
                    ':horas' => $horas 
    ...
    {
        $conexion = Database::getInstance();
        $sql = "SELECT id,nombre,horas from materias where id=:id"; 
        $result = $conexion->db->prepare($sql);
        $params = array("id" => $id);
        $result->execute($params);
        return $result;
    ...
    public function updateMateria($nombre,$horas,$id)  
    {
        try {
            $conexion = Database::getInstance();
            $result = $conexion->db->prepare("UPDATE materias set nombre=:nombre,horas=:horas where id=:id ");
            $result->execute(
                array(
                    ':nombre' => $nombre,
                    ':horas' => $horas, 
